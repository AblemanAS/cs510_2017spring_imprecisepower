...
inline __device__  float3 sum3(const float3 &a, const float3 &b){

	return make_float3( sum(a.x,b.x), sum(a.y,b.y), sum(a.z, b.z) );
}

inline __device__  float3 sub3(const float3 &a, const float3 &b){

	return make_float3( sub(a.x,b.x), sub(a.y,b.y), sub(a.z, b.z) );
}

inline __device__  float3 mul3(const float3 &a, const float3 &b){
	return make_float3( mul(a.x,b.x), mul(a.y,b.y), mul(a.z, b.z) );
}

inline __device__  float4 sum4(const float4 &a,  const float4 &b){
	return make_float4( sum(a.x,b.x), sum(a.y,b.y), sum(a.z, b.z), sum(a.w, b.w) );
}

inline __device__  float4 sub4(const float4 &a, const float4 &b){
	return make_float4( sub(a.x,b.x), sub(a.y,b.y), sub(a.z, b.z), sub(a.w, b.w) );
}

inline __device__  float4 mul4(const float4 &a, const float4 &b ){
	return make_float4( mul(a.x,b.x), mul(a.y,b.y), mul(a.z, b.z), mul(a.w, b.w) );
}
...
#endif
{
	...
	if( x < imageW && y < imageH )
	{
		...
		R.A = make_float3(M.m[0].w,M.m[1].w,M.m[2].w);
		R.u = make_float3(M.m[0])*df
			+ make_float3(M.m[2])*( sub(float(x), mul(float(imageW),0.5f)) )*tPixel
			+ make_float3(M.m[1])*( sub(float(y), mul(float(imageH),0.5f)) )*tPixel;
		R.u = normalize(R.u);
#ifdef DEVICE_EMU
...
//             printf("%d: i=%d, pile[i] = %e %e %e %e\n", threadIdx.x, i, pile[i].x, pile[i].y, pile[i].z, pile[i].w);
               ...
               #endif
					pile[i].x = sum(pile[i].x, tmp);
					pile[i].y = sum(pile[i].y, tmp);
					pile[i].z = sum(pile[i].z, tmp);
               #ifdef DEVICE_EMU
//             printf("%d: i=%d, pile[i] = %e %e %e %e\n", threadIdx.x, i, pile[i].x, pile[i].y, pile[i].z, pile[i].w);
               #endif
               #ifdef DEBUG_RT_CUDA
