	...
	__syncthreads();
        ...
        for (int i=0; i<iteration ; i++){ 
            ...
            if( IN_RANGE(tx, i+1, BLOCK_SIZE-i-2) &&  \
                  ...
                  computed = true;
                  temp_t[ty][tx] =   sum(temp_on_cuda[ty][tx],mul(step_div_Cap, 

sum(sum(power_on_cuda[ty][tx],mul(sum(temp_on_cuda[S][tx], sub(temp_on_cuda[N][tx],mul(2.0,temp_on_cuda[ty][tx]))), Ry_1)), sum(mul(sum(temp_on_cuda[ty][E],sub(temp_on_cuda[ty][W],mul(2.0,temp_on_cuda[ty][tx]))),Rx_1),mul(sub(amb_temp,temp_on_cuda[ty][tx]),Rz_1)))));
